...
MAX_ITERATIONS = 5                 # max number of self improving iterations. Never exceed them  

import logging
import inspect  # Added for my_name lambda

class ResultFormat(BaseLine):
    "success" : Bool = Field(..., description:"'False' indicates fatal error. 'True' indicates success."
    "data"    : Any = Field(..., description:"if failure, contains fatal error message. If success, contains results or non-fatal error message")
...
AGENT_NAMES_TO_MODELS = {
    ...
    {
     "product_owners":  {"po1" : {model: "openai:gpt-4o-mini"}, "po2" : {"model": "google_genai:gemini-2.0-flash"}
    }, 
    ...
    { 
     "developers" :    {"dev1" : {"model" : "openai:gpt-4o-mini"}}, 
    ...
    { 
      "testers" :       {"tester1" : {"model" : "openai:gpt-4o-mini"}}, 
